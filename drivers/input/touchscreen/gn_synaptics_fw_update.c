		...
		const unsigned char *fw_image)
{
	...
	if (header->is_contain_build_info) {
		...
		dev_info(&fwu->rmi4_data->i2c_client->dev,
			"tpd %s Firwmare build id %d\n", __func__,
			header->firmware_id);
	}

#ifdef DEBUG_FW_UPDATE
	dev_info(&fwu->rmi4_data->i2c_client->dev,
		"tpd Firwmare size %d, config size %d\n",
		header->image_size,
		header->config_size);
#endif
	return;
...
static int fwu_reset_device(void)
{
	...
	if (retval < 0) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"tpd %s: Failed to reset core driver after reflash\n",
				__func__);
		return retval;
	}
	return 0;
...
static enum flash_area fwu_go_nogo(struct image_header *header)
{
	...
	if (retval < 0) {
		dev_err(&i2c_client->dev,
			"tpd %s: Failed to read firmware ID (code %d).\n",
			__func__, retval);
		goto exit;
	}
	firmware_id[3] = 0;
	...
	if (header->is_contain_build_info) {
		dev_err(&i2c_client->dev,
			"tpd %s: Image option contains build info.\n",
			__func__);
		...
		if (!strptr) {
			dev_err(&i2c_client->dev,
				"tpd No valid PR number (PRxxxxxxx)" \
	...
	if (imageFirmwareID > deviceFirmwareID) {
		...
		flash_area = NONE;
		printk("tpd %s: Img fw is older than device fw. Skip fw update.\n", __func__);
		goto exit;
	}
	...
	if (retval < 0) {
		dev_err(&i2c_client->dev,
			"tpd %s: Failed to read config ID (code %d).\n",
			__func__, retval);
		flash_area = NONE;
		goto exit;
	}
	...
	printk("tpd Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		config_id[0], config_id[1], config_id[2], config_id[3]);

	/* .img config id */
	printk("tpd .img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
			fwu->config_data[0],
			fwu->config_data[1],
			fwu->config_data[2],
			fwu->config_data[3]);
	imageConfigID =  extract_uint_be(fwu->config_data);
       ...
       printk("tpd %s: Device config ID %d, .img config ID %d\n",
...
exit:
	...
	if (flash_area == NONE)
		dev_info(&i2c_client->dev,
			"tpd %s: Nothing needs to be updated\n", __func__);
	else
		dev_info(&i2c_client->dev,
			"tpd %s: Update %s block\n", __func__,
...
#ifdef DEBUG_FW_UPDATE
	dev_info(&fwu->rmi4_data->i2c_client->dev, "tpd Scan PDT\n");
#endif
...
static int fwu_do_reflash(void)
{
	...
	dev_dbg(&fwu->rmi4_data->i2c_client->dev,
			"tpd %s: Erase all command written\n",
			__func__);
	...
	if (fwu->flash_control.status != 0x00) {
		dev_err(&fwu->rmi4_data->i2c_client->dev,
				"tpd %s: Erase all command failed, status 0x%02X\n",
				__func__, retval);
		return -1;
	}
	...
	if (fwu->firmware_data) {
		...
		if (retval < 0)
			return retval;
		pr_notice("tpd %s: Firmware programmed\n", __func__);
	}
	...
	if (fwu->config_data) {
		...
		if (retval < 0)
			return retval;
		pr_notice("tpd %s: Configuration programmed\n", __func__);
	}
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->ext_data_source) {
		...
		if (!fwu->firmware_name) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
				"tpd %s Failed to allocate firmware name (%d).\n",
		...
		if (retval != 0) {
			dev_err(&fwu->rmi4_data->i2c_client->dev,
					"tpd %s: Firmware image %s not available\n",
					__func__,
					fwu->firmware_name);
			retval = -EINVAL;
			goto exit;
	...
	if (fwu->ext_data_source) {
              printk("tpd flash_area = UI_FIRMWARE\n");
		flash_area = UI_FIRMWARE;
       }
	else
	...
	dev_info(&fwu->rmi4_data->i2c_client->dev, "tpd Device is in %s mode\n",
		f01_device_status.flash_prog == 1 ? "bootloader" : "UI");
	if (f01_device_status.flash_prog)
		dev_info(&fwu->rmi4_data->i2c_client->dev, "tpd Flash status %d\n",
				f01_device_status.status_code);

	if (f01_device_status.flash_prog) {
		dev_info(&fwu->rmi4_data->i2c_client->dev,
				"tpd %s: Device is in flash prog mode 0x%02X\n",
	...
	pr_notice("tpd %s: End of reflash process\n", __func__);
       fwu->rmi4_data->reset_fw(fwu->rmi4_data);
exit:
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!fwu) {
		dev_err(&rmi4_data->i2c_client->dev,
				"tpd %s: Failed to alloc mem for fwu\n",
				__func__);
		goto exit;
	}
	...
	if (!fwu->fn_ptr) {
		dev_err(&rmi4_data->i2c_client->dev,
				"tpd %s: Failed to alloc mem for fn_ptr\n",
				__func__);
		retval = -ENOMEM;
		goto exit_free_fwu;
	}
	...
	if (retval < 0) {
		dev_dbg(&rmi4_data->i2c_client->dev,
				"tpd %s: Failed to read PDT properties, assuming 0x00\n",
				__func__);
	} else if (pdt_props.has_bsr) {
		dev_err(&rmi4_data->i2c_client->dev,
				"tpd %s: Reflash for LTS not currently supported\n",
				__func__);
		goto exit_free_mem;
	}
	...
	dev_dbg(&rmi4_data->i2c_client->dev,
			"tpd %s: F01 product info: 0x%04x 0x%04x\n",
			__func__, fwu->productinfo1, fwu->productinfo2);
	dev_dbg(&rmi4_data->i2c_client->dev,
			"tpd %s: F01 product ID: %s\n",
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev,
				"tpd %s: Failed to create sysfs bin file\n",
				__func__);
		goto exit_free_mem;
	}
